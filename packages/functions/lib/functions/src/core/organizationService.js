...
exports.reactivateOrganization = exports.suspendOrganization = exports.listOrganizations = exports.getOrganization = exports.updateOrganization = exports.createOrganization = void 0;
 ...
 */
exports.createOrganization = functions.https.onCall(async (request) => {
    ...
    const orgData = {
        ...
        trialEndsAt: admin.firestore.Timestamp.fromDate(new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) // 30 dias
        ),
    };
    ...
    try {
        ...
        return {
            ...
            organization: Object.assign({ id: orgId }, orgData),
        };
    }
    catch (error) {
 ...
 */
exports.updateOrganization = functions.https.onCall(async (request) => {
    const { auth, data } = request;
    if (!auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuário não autenticado');
    }
    ...
    if (user.role !== 'superadmin') {
        for (const field of superadminOnlyFields) {
            if (data.updates.hasOwnProperty(field)) {
                delete data.updates[field];
            }
        }
    }
    const updates = Object.assign(Object.assign({}, data.updates), { updatedAt: admin.firestore.FieldValue.serverTimestamp() });
 ...
 * Obter organização
 */
exports.getOrganization = functions.https.onCall(async (request) => {
    const { auth, data } = request;
    if (!auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuário não autenticado');
    }
 ...
 */
exports.listOrganizations = functions.https.onCall(async (request) => {
    const { auth, data } = request;
    if (!auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuário não autenticado');
    }
    ...
    const organizations = snapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data())));
    // Contar total
 ...
 */
exports.suspendOrganization = functions.https.onCall(async (request) => {
    const { auth, data } = request;
    if (!auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuário não autenticado');
    }
 ...
 */
exports.reactivateOrganization = functions.https.onCall(async (request) => {
    const { auth, data } = request;
    if (!auth) {
        throw new functions.https.HttpsError('unauthenticated', 'Usuário não autenticado');
    }
...
async function createDefaultSettings(organizationId) {
    const batch = db.batch();
